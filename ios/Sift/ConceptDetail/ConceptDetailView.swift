...
struct ConceptDetailView: View {
    ...
    /// In-memory only: the user's original capture text for this navigation.
    private let captureFallbackText: String?

    init(
        conceptId: UUID,
        initialMode: ConceptDetailMode = .overview,
        captureFallbackText: String? = nil
    ) {
        self.conceptId = conceptId
        self.captureFallbackText = captureFallbackText
    ...
    /// The original capture query, from the route (preferred) or a stored
    /// conversation. Never inferred from `displayTitle`.
    private var resolvedCaptureFallback: String? {
        if let text = captureFallbackText?.trimmingCharacters(in: .whitespacesAndNewlines),
           !text.isEmpty {
            return text
        }
        if let initial = concept?.conversation?.initialQuery
            .trimmingCharacters(in: .whitespacesAndNewlines), !initial.isEmpty {
            return initial
        }
        return nil
    }

    /// Show the temporary capture bubble only while no real user turn matches it.
    private var showCaptureFallback: Bool {
        guard let fallback = resolvedCaptureFallback else { return false }
        let key = fallback.lowercased()
        let hasRealUserTurn = turns.contains { turn in
            turn.role == ConversationRole.user.rawValue
                && turn.content.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == key
        }
        return !hasRealUserTurn
    }
                        ...
                        VStack(alignment: .leading, spacing: 20) {
                            ...
                            if detailMode == .overview {
                                ...
                                ConceptFollowUpView(
                                    ...
                                    captureFallback: showCaptureFallback ? resolvedCaptureFallback : nil
                                )
                            }
                            Color.clear
                                .frame(height: 1)
    ...
    private func restoreFailedFollowUpDraft() {
        ...
        // A failed follow-up draft is a user message stored with an internal
        // sentinel update mode (i.e. not a known `UpdateMode`). Read it directly
        // so opening a card never has to create an empty conversation.
        let draft = messages
            .filter { message in
                message.role == ConversationRole.user.rawValue
                    && UpdateMode(rawValue: message.updateMode) == nil
            }
    ...
    private func refreshTurns(_ conceptId: UUID) async {
        if let concept, ConceptStatusRules.isLocalOnly(concept.captureStatus) {
            turns = localConversationTurns(for: concept)
            return
        }
        do {
            ...
            if !remoteTurns.isEmpty {
                ...
                let localTurns = localConversationTurns(for: concept)
    ...
    /// Build conversation turns from locally stored messages. Only messages with
    /// a known `UpdateMode` are rendered; unknown modes are internal sentinels
    /// (e.g. failed follow-up drafts) handled by `restoreFailedFollowUpDraft()`.
    private func localConversationTurns(for concept: Concept) -> [ConceptHistoryTurnDTO] {
        guard let messages = concept.conversation?.messages else { return [] }
        return messages
            .sorted { $0.createdAt < $1.createdAt }
            .compactMap { message in
                guard UpdateMode(rawValue: message.updateMode) != nil,
                      ConversationRole(rawValue: message.role) != nil else {
                    return nil
                }
                return ConceptHistoryTurnDTO(role: message.role, content: message.content)
            }
    }
