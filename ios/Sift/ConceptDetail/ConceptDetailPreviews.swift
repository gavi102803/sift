    ...
    static func container(
        ...
        if withConversation {
            ...
            context.insert(ConversationMessage(
                ...
                content: "How is it different from a normal cache?",
                updateMode: UpdateMode.none.rawValue,
                conversation: conversation
            ))
            context.insert(ConversationMessage(
                role: ConversationRole.assistant.rawValue,
                content: "A normal cache needs an exact key match. A semantic cache matches on meaning, so paraphrases still hit.",
                updateMode: UpdateMode.none.rawValue,
                conversation: conversation
            ))
        }
...
#Preview("Follow-up · Conversation") {
    ...
    .modelContainer(ConceptPreview.container(withConversation: true))
    .environment(\.appServices, ConceptPreview.services())
    .preferredColorScheme(.dark)
}
...
#Preview("Follow-up · Generating") {
    NavigationStack {
        ConceptDetailView(
            conceptId: ConceptPreview.conceptId,
            initialMode: .followUp,
            captureFallbackText: "What is a semantic cache?"
        )
    }
    .modelContainer(ConceptPreview.container(status: .generating, withRelations: false))
...
#Preview("Follow-up · Generation failed") {
    ...
    .modelContainer(ConceptPreview.container(status: .generationFailed, withRelations: false))
